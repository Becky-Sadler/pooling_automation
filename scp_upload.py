...
def connect_via_ssh(host, username, SSH_path):
    SSHclient = paramiko.SSHClient()
    SSHclient.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    SSHclient.connect(hostname=host, username=username, key_filename=SSH_path, port = 22)
    return SSHclient

ssh = connect_via_ssh(host, username, SSH_path)
